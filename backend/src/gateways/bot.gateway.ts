  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleBotMessage(
    ...
    try {
      ...
      const followUpResponse = await this.handleFollowUpIfNeeded(
        client,
        text,
        context,
      );
  ...
  private async handleFollowUpIfNeeded(
    client: Socket,
    text: string,
    ...
    if (context.lastIntent === 'tour_search' && context.lastResults) {
      ...
      if (tourMatch) {
        const response = this.handleTourFollowUp(
          tourMatch[1],
          context.lastResults,
        );
    ...
    switch (intent) {
      ...
      case 'tour_search': {
        ...
        if ((this.nlpService as any).lastMatchedTrainingPhrase) {
          ...
          intro =
            this.responseService.getFallbackFromTrainingPhrase(
              match.phrase,
              match.city,
            ) + '\n\n';
        }
        ...
        return client.emit('bot:response', {
          text:
            intro +
            this.responseService.formatTourResults(results, {
              mentionedCity: searchCity,
            }),
          results,
          followUp: true,
        });
      ...
      default:
        return client.emit('bot:response', {
          text:
            llmResponse ||
            "I specialize in finding tours. Try something like: 'Find historical tours in Berlin'",
          followUp: false,
        });
    }
  }
  ...
  private async handleTourSearch(client: Socket, query: string, city: string) {
    ...
    try {
      ...
      return client.emit('bot:response', {
        text: this.responseService.formatTourResults(results, {
          mentionedCity: searchCity,
        }),
        results,
        followUp: true,
  ...
  async handleFeedback(
    ...
    try {
      ...
      client.emit('bot:response', {
        text: feedback.isHelpful
          ? 'Thanks for your feedback! 😊'
          : 'Sorry it wasn’t helpful. We’ll improve. 🙏',
        //   followUp: true
      });
